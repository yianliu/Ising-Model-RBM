...
for i in range(nt):
    ...
    for lr in lr_list:
        ...
        completeName = os.path.join('Data', 'Learning Rate Test', data_path, file_name)
    ...
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'E_vals.npy'), E_vals)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'E_errs.npy'), E_errs)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'M_vals.npy'), M_vals)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'M_errs.npy'), M_errs)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'Cv_vals.npy'), Cv_vals)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'Cv_errs.npy'), Cv_errs)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'X_vals.npy'), X_vals)
    np.save(os.path.join(obs_path, 'T = ' + format(T, '.2f') + 'X_errs.npy'), X_errs)
